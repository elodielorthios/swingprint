...
function setupOsc(oscPortIn, oscPortOut) {
	var socket = io.connect('http:/192.168.43.248:8081', { port: 8081, rememberTransport: false });
	socket.on('connect', function() {
		socket.emit('config', {
			server: { port: oscPortIn,  host: '192.168.43.248'},
			client: { port: oscPortOut, host: '192.168.43.248'}
		});
	});
